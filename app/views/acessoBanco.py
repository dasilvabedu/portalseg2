...
def chatterbot(pergunta):
    ...
    bot = ChatBot("midiasii", storage_adapter="chatterbot.storage.SQLStorageAdapter", database_uri=database_url)
    conta = 0
...
def insereMetadado(tabela, atual_data, atual_usuario):
    ...
    try:
        ...
        colunas = (
            "SELECT column_name, ordinal_position, data_type FROM information_schema.columns WHERE table_name = '"
            + tabela
            + "'"
        )
        cur.execute(colunas)
        ...
        for dados in dadosAtributo:
            ...
            campos = " (mtt_identificador, mta_identificador, mta_atributo, mta_sequencia, mta_tipo, mta_editavel,"
            campos = campos + " mta_identificadoratualizacao, mta_dataatualizacao)"

            valores = (
                " values ("
                + str(proximoNumeroMetadado)
                + ","
                + str(atualNumeroAtributo)
                + ",'"
                + dados[0]
                + "',"
                + str(dados[1])
...
def exclueMetadadoAtributo(lista):
    ...
    try:
        ...
        for k in lista:
            ...
            # deleta o atributo
            comando = (
                "DELETE FROM mta_metadadoatributo WHERE mtt_identificador = "
                + str(mtt_identificador)
                + " AND mta_atributo = '"
                + atributo
                + "'"
            )
...
def insereUsuario(usu_celular, usu_email, usu_senha, usu_nome):
    ...
    try:
        ...
        campos = "usu_identificador, usu_senha, usu_autenticacao, usu_nome, usu_identificadoratualizacao, "
        campos = campos + "usu_dataatualizacao "

        valores = (
            str(proximoNumero)
            + ",'"
            + str(senhaCriptografada)
            + "','ativo','"
            + usu_nome
            + "',"
            + str(proximoNumero)
        ...
        condicao = (
            ...
            + "WHERE pfa_perfilacesso.pfa_identificador = 2"
        )
