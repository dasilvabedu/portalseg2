...
@app.route("/seguranca_barragem/rotastotal/<int:id>", methods=["GET"])
def api_rotas_total(id):
    resultado, retorno, header = gestaoUHE.rotasTotal(id)
    return jsonify(resultado), retorno, header
...
@app.route("/seguranca_barragem/pontosrota/<int:id>", methods=["GET"])
def api_pontosrotas(id):
    resultado, retorno, header = gestaoUHE.pontosRotaTotal(id)
    return jsonify(resultado), retorno, header


@app.route("/seguranca_barragem/pontorota/<int:id>", methods=["GET"])
def api_pontorota_especifico(id):
    resultado, retorno, header = gestaoUHE.pontoRotaEspecifico(id)
    return jsonify(resultado), retorno, header
