...
def metadadoSelecionado():
    ...
    resultadoTabela, resultadoAtributo, retorno, mensagemRetorno = acessoBanco.leMetadado(
        mtt_identificador, mtt_tabela, camposDesejados, None
    )
...
def metadadoQualificado():
    ...
    resultadoTabela, resultadoAtributo, retorno, mensagemRetorno = acessoBanco.leMetadado(
        mtt_identificador, mtt_tabela, None, "Sim"
    )
    resultadoFinal = acessoBanco.montaRetorno(retorno, mensagemRetorno)
    if retorno != 200:
        return resultadoFinal, retorno, header
...
def metadadoValidado():
    ...
    for k in range(len(listaTabelas)):
        if listaTabelas[k] not in listaMetadados:
            inseridos, retorno, mensagemRetorno = acessoBanco.insereMetadado(
                listaTabelas[k], atual_data, atual_usuario
            )
    ...
    for k in range(len(metadados)):
        ...
        information, retorno, mensagemRetorno = acessoBanco.dado(
            "information_schema.columns", condicao, camposInformation, None
        )
        resultadoFinal = acessoBanco.montaRetorno(retorno, mensagemRetorno)
        if retorno != 200:
            return resultadoFinal, retorno, header
    ...
    if atributosIncluir != []:
        atributos, retorno, mensagemRetorno = acessoBanco.insereMetadadoAtributo(
            atributosIncluir, atual_data, atual_usuario
        )
        if retorno != 201:
            return resultadoFinal, retorno, header
