...
def listaModulo():
    camposDesejados = (
        "moc_identificador, moc_sigla, moc_nome, moc_identificadoratualizacao, moc_dataatualizacao,usu_nome"
    )
...
def moduloEspecifico(id):
    ...
    for i in range(len(dadosModulo)):
        ...
        condicao = (
            "INNER JOIN trc_trilhacapacitacao ON "
            + "trc_trilhacapacitacao.trc_identificador = trc_moc_trilhacapacitacao_modulocapacitacao.trc_identificador "
            + "WHERE moc_identificador = " + str(id)
        )
        dadosTrilha, retorno, mensagemRetorno = acessoBanco.leDado(
            "trc_moc_trilhacapacitacao_modulocapacitacao", condicao, camposDesejados
        )
        if retorno == 400:
            return False, {"message": "Erro no acesso ao banco de dados"}, {}
...
def trataModuloDeletado(identificador):
    ...
    condicao = "WHERE moc_identificador = " + str(identificador)
    dados, retorno, header = acessoBanco.leDado(
        "trc_moc_trilhacapacitacao_modulocapacitacao", condicao, camposDesejados
    )
...
def listaTrilha():
    ...
    condicao = (
        "INNER JOIN usu_usuario ON usu_identificador = trc_trilhacapacitacao.trc_identificadoratualizacao "
        + "INNER JOIN pfu_perfilusuario ON pfu_perfilusuario.pfu_identificador = "
        + "trc_trilhacapacitacao.pfu_identificador "
    )
...
def trilhaEspecifica(id):
    ...
    condicao = (
        ...
        + "INNER JOIN pfu_perfilusuario ON pfu_perfilusuario.pfu_identificador = "
        + "trc_trilhacapacitacao.pfu_identificador "
        + "WHERE trc_identificador = " + str(id)
    )
    ...
    for i in range(len(dadosTrilha)):
        ...
        condicao = (
            "INNER JOIN moc_modulocapacitacao ON "
            + "moc_modulocapacitacao.moc_identificador = trc_moc_trilhacapacitacao_modulocapacitacao.moc_identificador "
            + "WHERE trc_identificador = " + str(id)
        )
        dadosTrilha, retorno, mensagemRetorno = acessoBanco.leDado(
            "trc_moc_trilhacapacitacao_modulocapacitacao", condicao, camposDesejados
...
def trataTrilhaIncluida(usu_identificador, usu_nome, sigla, nome, perfil):
    ...
    agora = datetime.datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S")
    camposDesejados = (
        "trc_identificador, trc_sigla, trc_nome, trc_identificadoratualizacao, trc_dataatualizacao, pfu_identificador"
    )
    valores = (
        str(proximoNumero)
        + ",'"
...
def trataTrilhaQualificaIncluida(id, usu_identificador, usu_nome, modulo, seq):
    ...
    agora = datetime.datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S")
    camposDesejados = (
        "trc_identificador, moc_identificador, trc_moc_sequencia, trc_moc_identificadoratualizacao, "
        + "trc_moc_dataatualizacao"
    )
    valores = str(id) + "," + str(modulo) + "," + str(seq) + "," + str(usu_identificador) + ",'" + agora + "'"
    dados, retorno, header = acessoBanco.insereDado(
        "trc_moc_trilhacapacitacao_modulocapacitacao", camposDesejados, valores
    )
