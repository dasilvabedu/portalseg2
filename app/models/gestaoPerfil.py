...
def qualificadorNovoLista(grupo):
    ...
    if request.method == 'POST':
        ...
        resultadoFinal, retorno = trataQualificadorIncluido(grupo, dadosToken["sub"], dadosToken['name'], titulo, descricao)
...
def qualificadorAtual(grupo, id):
    ...
    if request.method == 'PATCH':
        ...
        titulo = entrada.get('titulo')
        descricao = entrada.get('descricao')
        resultadoFinal, retorno = trataQualificadorAlterado(grupo, dadosToken['sub'], dadosToken['name'], id, titulo, descricao)
        return resultadoFinal, retorno, header
...
def perfilNovoLista():
    ...
    if request.method == 'POST':
        ...
        resultadoFinal, retorno = trataPerfilIncluido(dadosToken["sub"], dadosToken['name'],sigla, descricao, qual_um, qual_dois, qual_tres, qual_quatro)
        return resultadoFinal, retorno, header
    elif request.method == 'GET':
...
def perfilAtual(id):
    ...
    if request.method == 'PATCH':
        ...
        qual_tres = entrada.get('qualificador_tres')
        qual_quatro = entrada.get('qualificador_quatro')
        resultadoFinal, retorno = trataPerfilAlterado(id, dadosToken["sub"], dadosToken['name'], sigla, descricao, qual_um,
                                                      qual_dois, qual_tres, qual_quatro)
        return resultadoFinal, retorno, header
...
def trataPerfilIncluido(usu_identificador, usu_nome, sigla, descricao, qual_um, qual_dois, qual_tres, qual_quatro):
    ...
    condicao = condicao + " and qlt_identificador = " + str(qual_tres) + " and qlq_identificador = " + str(qual_quatro)  + ")"
    dados, retorno, mensagemRetorno = acessoBanco.leDado('pfu_perfilusuario', condicao, camposDesejados)
    if retorno == 400:
        return {"message": "Erro no acesso ao banco de dados"}, retorno
...
def trataPerfilAlterado(id,usu_identificador, usu_nome, sigla, descricao, qual_um, qual_dois, qual_tres, qual_quatro):
    ...
    camposDesejados = 'pfu_sigla, pfu_descricao, qlu_identificador, qld_identificador, qlt_identificador, qlq_identificador'
    ...
    agora = datetime.datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S')
    condicao = "WHERE pfu_identificador = " + str(id)
    valores = "pfu_sigla = '" + sigla + "', pfu_descricao = '" + descricao + "', qlu_identificador = " + str(qual_um) + ","
...
def trataAcessoQualificaIncluido(id, usu_identificador, usu_nome, transacao):
    ...
    dados, retorno, mensagemRetorno = acessoBanco.leDado('pfa_trn_perfilacesso_transacaosistema', condicao, camposDesejados)
    if retorno == 400:
        return {"message": "Erro no acesso ao banco de dados"}, retorno
